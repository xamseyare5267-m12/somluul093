...
// Arabic Dictionary
const arDict = {
  nav_feed: 'الصفحة الرئيسية',
  nav_messenger: 'الرسائل',
  nav_marketplace: 'السوق',
  nav_monetization: 'مركز المبدعين',
  nav_storage: 'التخزين السحابي',
  nav_admin: 'لوحة الإدارة',
  nav_downloads: 'تحميل التطبيقات',
  nav_support: 'مركز المساعدة',
  nav_logout: 'تسجيل الخروج',
  welcome_back: 'مرحبًا بك في SomLuul',
  signin_desc: 'سجل الدخول للتواصل مع شبكة SomLuul العالمية',
  signup_desc: 'أنشئ حسابًا لبدء المشاركة والاستكشاف',
  ...
  verify_desc: 'تم إرسال بريد إلكتروني للتحقق إلى عنوانك.',
  verify_btn: 'تحقق وتابع',
  forgot_pass: 'نسيت كلمة المرور؟',
  reset_pass: 'إعادة تعيين كلمة المرور',
  feed_title: 'الخلاصة العامة',
  mind_placeholder: 'ما الذي يدور في ذهنك اليوم؟',
  post_btn: 'نشر',
  ...
  stories_title: 'القصص',
  reels_title: 'مقاطع الفيديو والبث',
  chats_title: 'المحادثات',
  secret_chat: 'محادثة سرية مشفرة',
  search_chats: 'ابحث في الرسائل والأصدقاء...',
  type_message: 'اكتب رسالة بأمان...',
  voice_msg: 'تسجيل رسالة صوتية',
  typing: 'يكتب الآن...',
  online_now: 'متصل الآن',
  last_seen: 'آخر ظهور مؤخرًا',
  call_voice: 'مكالمة صوتية HD',
  call_video: 'مكالمة فيديو HD',
  group_call: 'مكالمة جماعية',
  screen_share: 'مشاركة الشاشة',
  end_call: 'إنهاء المكالمة',
  marketplace_title: 'سوق SomLuul',
  all_categories: 'جميع الفئات',
  sell_btn: 'بيع منتج',
  price: 'السعر',
  location: 'الموقع',
  item_title: 'اسم المنتج',
  item_desc: 'الوصف',
  message_seller: 'مراسلة البائع',
  post_item: 'عرض المنتج',
  creator_title: 'مركز المبدعين والأعمال',
  wallet_bal: 'رصيد المحفظة',
  payout_btn: 'سحب الأرباح',
  friends: 'الأصدقاء',
  ...
  profile_videos: 'الفيديوهات القصيرة',
...
const supportedLangs = [
  'en', 'so', 'ar', 'fr', 'es', 'pt', 'de', 'it', 'tr', 'ru',
  'zh-CN', 'zh-TW', 'ja', 'ko', 'hi', 'ur', 'fa', 'id', 'ms',
  'nl', 'sv', 'no', 'da', 'fi', 'pl', 'uk', 'el', 'he', 'th',
  'vi', 'bn', 'sw', 'am', 'ha', 'yo', 'zu'
];

fs.writeFileSync(path.join(localesDir, 'en.json'), JSON.stringify(enDict, null, 2), 'utf8');
fs.writeFileSync(path.join(localesDir, 'so.json'), JSON.stringify(soDict, null, 2), 'utf8');
fs.writeFileSync(path.join(localesDir, 'ar.json'), JSON.stringify({ ...enDict, ...arDict }, null, 2), 'utf8');

// For other languages, generate based on enDict with fallback
for (const lang of supportedLangs) {
  const filePath = path.join(localesDir, `${lang}.json`);
  if (!fs.existsSync(filePath)) {
    fs.writeFileSync(filePath, JSON.stringify(enDict, null, 2), 'utf8');
  }
}
...
const icon120 = path.join(publicDir, 'icon-120x120.png');
if (fs.existsSync(icon120)) {
  const iconData = fs.readFileSync(icon120);
  const iconNames = [
    'icon-152x152.png',
    'icon-167x167.png',
    'icon-180x180.png',
  ...
  for (const name of iconNames) {
    ...
    if (!fs.existsSync(p)) fs.writeFileSync(p, iconData);
  }
}

console.log('Locales & assets built successfully!');
